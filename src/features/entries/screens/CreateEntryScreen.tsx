...
export function CreateEntryScreen() {
  ...
  const { tags, fetchTags, searchTags } = useTags();
  ...
  const setupVoiceRecognition = useCallback(() => {
    ...
    Voice.onSpeechResults = (e: any) => {
      ...
      if (text) {
        setValue('content', contentValue + (contentValue ? ' ' : '') + text);
      }
    };
    Voice.onSpeechError = (error: any) => {
      Alert.alert('Error', `Speech recognition error: ${error.error}`);
  ...
  const requestLocation = useCallback(async () => {
    try {
      ...
      setLocationText(address || `${latitude.toFixed(4)}, ${longitude.toFixed(4)}`);
    } catch (error) {
  ...
  const onSubmit = async (data: EntryFormData) => {
    const entry = await createEntry({
      ...
      audioUri: selectedMedia.audioUri,
        ...
        >
          ...
          >
            <Text
              style={[
                theme.typography.body,
                {
                  color: locationText ? theme.colors.primary : theme.colors.textSecondary,
                },
              ]}
            >
          ...
          <View style={styles.toolbar}>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <Text style={styles.toolbarIcon}>{isRecording ? '⏹️' : '🎙️'}</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.toolbarButton,
                {
                  borderColor: isTranscribing ? theme.colors.primary : theme.colors.border,
                  backgroundColor: isTranscribing ? theme.colors.primary + '20' : 'transparent',
                },
              ]}
              onPress={isTranscribing ? stopVoiceToText : startVoiceToText}
              accessible
              accessibilityLabel={isTranscribing ? 'Stop voice to text' : 'Start voice to text'}
              accessibilityRole="button"
            >
              <Text style={styles.toolbarIcon}>{isTranscribing ? '🎙️' : '🎤'}</Text>
            </TouchableOpacity>
